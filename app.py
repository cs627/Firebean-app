...
Content: Professional Business English for Slides. Slang TC for Social Media.
"""
...
def call_gemini_rest(prompt, image_b64=None, mode="text"):
    """使用 REST API 調用 Gemini，修復 403/404 報錯"""
    api_key = st.secrets.get("GEMINI_API_KEY", "")
    if not api_key:
        log_debug("SECRET ERROR: GEMINI_API_KEY missing!", "error")
        return None
    ...
    try:
        ...
        log_debug(f"API Failed: {str(e)}", "error")
    return None
...
def manna_ai_enhance(image_file):
    """真正 AI 影像擴展 (Outpainting) + 修正直相方向"""
    log_debug(f"Starting AI Expand for {image_file.name}...")
    with st.spinner("🚀 Manna AI 正在進行生成式影像擴展..."):
        try:
            ...
            result = call_gemini_rest(prompt, image_b64=b64, mode="image")
            if result:
                parts = result.get('candidates', [{}])[0].get('content', {}).get('parts', [])
                for part in parts:
                    if 'inlineData' in part:
                        img_data = base64.b64decode(part['inlineData']['data'])
                        log_debug("AI Expansion Success", "success")
                        return Image.open(io.BytesIO(img_data))
            log_debug("AI didn't return image, using original.", "warning")
            return img
        except Exception:
...
def main():
    ...
    t1, t2 = st.tabs(["💬 Data Collector", "📋 AI Review & Sync"])

    with t1:
        st.markdown('<div class="neu-card">', unsafe_allow_html=True)
        st.subheader("🎨 Logos (PNG)")
        ...
        with cl:
            ...
            if p := st.chat_input("詢問細節..."):
                ...
                res = call_gemini_rest(f"SOW: {st.session_state.scope_of_word}\nQuery: {p}")
                if res:
                    st.session_state.messages.append({"role": "assistant", "content": res.get('candidates', [{}])[0].get('content', {}).get('parts', [{}])[0].get('text', "AI Response failed.")})
                    st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)

        with cr:
            st.markdown('<div class="neu-card">', unsafe_allow_html=True)
            st.subheader("📸 Manna Gallery (AI Expand)")
    ...
    with t2:
        ...
        if st.button("🚀 Confirm & Sync"):
            ...
            payload = {
                ...
                "ai": st.session_state.ai_content, "images": b64s, "logo_w": st.session_state.logo_white, "logo_b": st.session_state.logo_black
            }
            res = requests.post(SHEET_SCRIPT_URL, json=payload, timeout=45)
            if "Success" in res.text: st.balloons(); st.success("✅ Sync Success!")
