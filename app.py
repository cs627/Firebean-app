...
CRITICAL: Return a single valid JSON OBJECT. 
Required Keys:
- "challenge_summary": 文字總結
- "solution_summary": 文字總結
- "1_google_slide": 簡報結構
- "2_facebook_post": FB 文案
- "3_threads_post": Threads 文案
- "4_instagram_post": IG 文案 (限 150 字)
- "5_linkedin_post": LinkedIn 文案
- "6_website": {"en": "...", "tc": "...", "jp": "..."}
"""
...
def call_gemini_sdk(prompt, image_files=None, is_json=False):
    """規格書 5.1 節：格式自動修復機制 (Format Fixer)"""
    ...
    try:
        ...
        if response and response.text:
            ...
            # 🚀 格式修復：如果是 List 則提取第一個項目
            try:
                data = json.loads(json_str)
                if isinstance(data, list) and len(data) > 0:
                    log_debug("⚠️ AI 回傳 List，已自動修正為 Dictionary", "warning")
...
def main():
    ...
    # 導航欄 (Fix Syntax Structure)
    st.markdown("<br>", unsafe_allow_html=True)
    tab_nav = st.columns(3)
    if tab_nav[0].button("📝 Project Collector", use_container_width=True, type="primary" if st.session_state.active_tab == "📝 Project Collector" else "secondary"):
        st.session_state.active_tab = "📝 Project Collector"; st.rerun()
    if tab_nav[1].button("📋 Review & Multi-Sync", use_container_width=True, type="primary" if st.session_state.active_tab == "📋 Review & Multi-Sync" else "secondary"):
        st.session_state.active_tab = "📋 Review & Multi-Sync"; st.rerun()
    if tab_nav[2].button("🛠️ Debug Terminal", use_container_width=True, type="primary" if st.session_state.active_tab == "🛠️ Debug Terminal" else "secondary"):
        st.session_state.active_tab = "🛠️ Debug Terminal"; st.rerun()

    # --- TAB 分頁內容 ---
    if st.session_state.active_tab == "📝 Project Collector":
        if st.button("🧪 老細一鍵填充 (規格書深度測試)", use_container_width=True):
        ...
        with cl:
            st.markdown('<div class="neu-card">', unsafe_allow_html=True)
            if st.button("🪄 生成 20 題繁中診斷"):
                if not st.session_state.project_photos: st.error("請先上傳相片。")
                else:
                    with st.spinner("AI 分析視覺事實中..."):
                        facts = call_gemini_sdk("Extract visual facts.", image_files=st.session_state.project_photos)
                        res = call_gemini_sdk(f"基於事實 {facts} 出題。格式: [{{\"id\":1,\"question\":\"...\",\"options\":[\"A\",\"B\"]}}]", is_json=True)
        ...
        with cr:
            st.markdown('<div class="neu-card">', unsafe_allow_html=True)
            fup = st.file_uploader("Upload 4-8 Photos", accept_multiple_files=True)
        ...
        if st.button("🪄 生成文案策略"):
            with st.spinner("AI Strategist 對位中..."):
                ...
                res = call_gemini_sdk(f"數據:{mc_sum}. 生成 JSON。IG < 150 字。", is_json=True)
                if res:
                    data = json.loads(res)
