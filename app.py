...
def call_gemini_rest(prompt, image_b64=None, mode="text"):
    ...
    # 這裡必須確保 secrets 有 GEMINI_API_KEY
    api_key = st.secrets.get("GEMINI_API_KEY", "")
    if not api_key:
        log_debug("SECRET ERROR: GEMINI_API_KEY is missing!", "error")
        st.error("請在 Streamlit Secrets 設置 GEMINI_API_KEY")
        return None
...
def manna_ai_enhance(image_file):
    """真正的 AI 影像擴展 (Outpainting) + 直相方向修正"""
    ...
    with st.spinner("🚀 Manna AI 正在進行生成式影像擴展..."):
        try:
            ...
            prompt = "Generatively outpaint this image into a cinematic 16:9 landscape banner. Naturally extend the environment and background to fill both sides. High-end lighting."
            ...
            if result:
                ...
                for part in parts:
                    if 'inlineData' in part:
                        ...
                        log_debug("AI Expansion Completed Successfully.", "success")
                        return Image.open(io.BytesIO(img_data))
            
            log_debug("AI response contained no image data. Using original.", "warning")
            return img
        except Exception:
...
def main():
    ...
    # Progress 計算 (11 維度：包含 What/SOW/Chatbot/Logos/Photos)
    ...
    with tab1:
        ...
        # 資訊區 (包含 What we do / Scope of work)
        st.markdown('<div class="neu-card">', unsafe_allow_html=True)
        ...
        with cr:
            ...
            if files:
                ...
                # 修正 Radio Index Error
                curr_idx = st.session_state.hero_index if st.session_state.hero_index < len(files) else 0
                hero_choice = st.radio("🌟 Hero Banner?", [f"P{i+1}" for i in range(len(files))], index=curr_idx, horizontal=True)
