...
1. Gap Analysis: Identify gaps between brand message and audience reception.
2. Spatial Logic: Analyze how decor and flow affect emotions.
3. Conversion Intent: Evaluate if the interactive design drives brand conversion.
4. Tech-Fit: Ensure technology (AR/VR/LED) serves the theme, not just a gimmick.
5. Operational ROI: Find subtle defects in execution (Souvenirs, KOL alignment).

OUTPUT GUIDELINES:
- Google Slide: Structured, authoritative, Hook/Shift/Proof.
- Instagram: STRICTLY UNDER 150 CHARACTERS, Emoji-rich.
- Website: Generate 3 versions: EN (Fluent), TC (Formal), JP (Polite).
"""
...
def call_gemini_sdk(prompt, image_files=None, is_json=False):
    """Gemini 2.5 SDK 調用 (具備 JSON 保護)"""
    secret_key = st.secrets.get("GEMINI_API_KEY", "")
    if not secret_key:
        log_debug("🚨 找不到 Secret Key", "error")
        return None
...
def fill_dummy_data():
    """🚀 老細專用：一鍵填充 100% 數據 (含 8 張圖片)"""
    st.session_state.client_name = "Firebean 旗艦測試"
    st.session_state.project_name = "2026 全自動同步閉環"
    ...
    st.session_state.open_question_ans = "透過 AI 技術達成秒速排版，解決人力資源斷層。"
    ...
    st.session_state.mc_questions = [
        {"id": i+1, "question": f"診斷維度測試題目 {i+1}？", "options": ["選項 A", "選項 B", "選項 C"]} 
        for i in range(20)
    ]
    for i in range(1, 21):
    ...
    log_debug("🚀 一鍵填充完成，準備觸發自動導向...", "success")
...
def get_animated_bar_html(percent, status_text):
    ...
    <div style="padding: 25px; background: #E0E5EC; border-radius: 15px; box-shadow: inset 6px 6px 12px #bec3c9, inset -6px -6px 12px #ffffff; margin: 20px 0;">
        <div style="font-weight: 800; color: #FF0000; font-size: 14px; text-align: center; margin-bottom: 10px; letter-spacing: 1px;">{status_text}</div>
        ...
        <div style="text-align: right; font-size: 11px; font-weight: 900; margin-top: 5px; color: #2D3436;">PROCESS: {percent}%</div>
    </div>
    """
...
def main():
    ...
    if st.session_state.active_tab == "📝 Project Collector":
        if st.button("🧪 老細一鍵填充：自動畫圖 8 張 + 填寫 20 MC", use_container_width=True):
            fill_dummy_data()
            st.rerun()
        ...
        with cl:
            ...
            if st.button("🪄 生成 20 題繁中診斷題目 (視覺 Facts 提取)"):
                if not st.session_state.project_photos: st.error("請上傳相片。")
                else:
                    loader = st.empty()
                    for p in range(0, 95, 5):
                        loader.markdown(get_animated_bar_html(p, "📸 正在掃描 Branding 及 空間細節..."), unsafe_allow_html=True)
                        time.sleep(0.04)
            ...
            if st.session_state.mc_questions:
                for i, q in enumerate(st.session_state.mc_questions):
                    ...
                    ans_key = f"ans_{q['id']}"
                    st.session_state[ans_key] = st.multiselect("選擇答案", q['options'], key=f"sel_{i}", default=st.session_state.get(ans_key, []))
            ...
            if st.session_state.project_photos:
                ...
                for i, f in enumerate(st.session_state.project_photos):
                    with g_cols[i%4]:
                        ...
                        except: st.image(f, use_container_width=True) # 支援 Dummy
            st.markdown('</div>', unsafe_allow_html=True)

    # --- TAB 2: REVIEW & SYNC ---
    elif st.session_state.active_tab == "📋 Review & Multi-Sync":
        st.markdown('<div class="neu-card">', unsafe_allow_html=True)
        if st.button("🪄 生成六大平台對接文案 (Institutional Cool Tone)"):
            loader = st.empty()
            for p in range(0, 96, 4):
                loader.markdown(get_animated_bar_html(p, "🧠 AI Strategist 正在處理三語 Website 及 IG 限制..."), unsafe_allow_html=True)
                time.sleep(0.04)
            
            mc_sum = [f"Q:{q['question']} A:{st.session_state.get(f'ans_{q['id']}')}" for q in st.session_state.mc_questions]
            prompt = f"數據:{mc_sum}. Generate JSON numbering keys 1_google_slide to 6_website. Website needs EN/TC/JP. IG < 150 chars. Include 'challenge_summary' and 'solution_summary'."
            res = call_gemini_sdk(prompt, is_json=True)
            if res:
        ...
        if st.session_state.ai_content:
            ...
            if st.button("🚀 Confirm & Sync (Master DB + Slide Page 1 & 2)", type="primary", use_container_width=True):
