...
# --- 1. 核心配置與 API Key 池 ---
# [cite_start]鎖定老細提供的新 GAS 同步 URL [cite: 1, 9-16]
...
# 🔑 鎖死從 Streamlit Secrets 讀取 API Key 池 (解決 403 Leaked 問題)
# 請在 Streamlit Cloud Settings -> Secrets 貼入: API_KEYS = ["Key1", "Key2", "Key3"]
...
def init_session_state():
    fields = {
        ...
        "youtube_link": "", "messages": [{"role": "assistant", "content": "Firebean Brain Online. 我係你嘅專屬 PR Director。今次個 Project 聽落好有潛力，可唔可以分享下最初 Client 遇到最大嘅 Challenge (痛點) 係咩？我哋一齊度橋點樣 Turn Policy into Play！"}],
...
def get_proactive_chatbot_prompt():
    ...
    You are a Proactive Senior PR Director. INTERVIEW Account Executive for: {missing}. 
    ...
    with tab1:
        ...
        with cl:
            ...
            if st.button("⏹️ 強制結束訪談並生成"):
                res = call_gemini_sdk("FORCE GENERATE FINAL JSON NOW based on current info.", is_json=True)
