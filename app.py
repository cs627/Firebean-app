...
- "challenge_summary": Brief summary of main hurdles.
- "solution_summary": Core strategic approach.
- "1_google_slide": Hook/Shift/Proof structure for presentation.
- "2_facebook_post": Engaging, story-driven with CTA.
- "3_threads_post": Humorous, critical, meme-potential.
- "4_instagram_post": Visual focus, emoji-rich, STRICTLY < 150 chars.
...
def call_gemini_sdk(prompt, image_files=None, is_json=False):
    ...
    try:
        ...
        if response and response.text:
            ...
            # 🚀 v2.6 核心修復：Format Fixer (List to Dict)
            try:
                data = json.loads(json_str)
                if isinstance(data, list) and len(data) > 0:
                    log_debug("⚠️ AI 回傳了 List 格式，自動轉換為 Dictionary。", "warning")
...
def init_session_state():
    """規格書 5.2 節：強制初始化所有變量，杜絕 AttributeError"""
    fields = {
...
def fill_dummy_data():
    ...
    log_debug("🚀 高質量測試數據填充完成，準備觸發 100% 自動導向。", "success")
...
def main():
    ...
    # 🎯 規格書 5.3 節：100% Drive Logic 自動跳轉
    ...
    # --- 分頁：COLLECTOR ---
    with tabs[0]:
        if st.button("🧪 老細一鍵填充 (規格書第 6 節內容)", use_container_width=True):
        ...
        with col1:
            ...
            if ub:
                ...
                st.session_state.logo_black = base64.b64encode(buf.getvalue()).decode()
        with col2:
            uw = st.file_uploader("Upload White Logo", type=['png'], key="l_w")
            if uw:
                img = Image.open(uw)
                buf = io.BytesIO(); img.save(buf, format="PNG")
                st.session_state.logo_white = base64.b64encode(buf.getvalue()).decode()
        ...
        with cl:
            ...
            if st.button("🪄 生成 20 條繁中診斷題目"):
                if not st.session_state.project_photos: st.error("請先上傳相片以供 Facts 提取。")
                else:
                    with st.spinner("AI 正在掃描空間與 Branding 事實..."):
                        facts = call_gemini_sdk("Identify branding, decor, and tech facts from these photos.", image_files=st.session_state.project_photos)
                        st.session_state.visual_facts = facts
                        prompt = f"基於事實: {facts}。生成 20 條繁中 MC 診斷題目。格式: [{{\"id\":1,\"question\":\"...\",\"options\":[\"A\",\"B\"]}}]"
    ...
    # --- 分頁：REVIEW & SYNC ---
        ...
        if st.button("🪄 生成六大平台對接文案"):
            with st.spinner("AI Strategist 正在根據診斷結果對位數據..."):
                mc_sum = [f"Q:{q['question']} A:{st.session_state.get(f'ans_{q['id']}')}" for q in st.session_state.mc_questions]
                prompt = f"數據對位分析: {st.session_state.project_name}. 回答摘要: {mc_sum}. 生成 6 個平台的繁中 JSON。"
                res = call_gemini_sdk(prompt, is_json=True)
                if res:
                    try:
                        data = json.loads(res)
                        if isinstance(data, dict):
                            ...
                            st.success("✅ 策略生成完成")
                        else: st.error("🚨 AI 回傳格式不符合預期 (非字典)")
            ...
            if st.button("🚀 Confirm & Sync (Sheet + Slide + Drive)", type="primary", use_container_width=True):
                with st.spinner("🔄 多軌同步中..."):
                    try:
                        ...
                        # 🚀 規格書 2.6 校準 Payload：包含 date, youtube, scope 以及 logo_white
                        payload = {
                            "action": "sync_project",
