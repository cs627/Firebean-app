...
def call_gemini_rest(prompt, image_b64=None, mode="text"):
    """使用 REST API 調用 Gemini，修正 404 路徑問題"""
    ...
    for i in range(3):
        try:
            ...
            elif response.status_code == 404 and "1.5" in model_id:
                # 如果 1.5 報 404，嘗試更換路徑或模型
                log_debug("Model not found, check model ID or API version.", "error")
                break
            else:
                log_debug(f"API Error {response.status_code}: {response.text[:150]}", "error")
            time.sleep(2)
...
def standardize_logo(logo_file, target_size=(800, 400), padding=40):
    ...
    try:
        ...
        img = ImageOps.exif_transpose(raw).convert("RGBA") # 修正方向
        bbox = img.getbbox()
        ...
        log_debug(f"Logo Standardizer Error: {str(e)}", "error")
        return ""

def manna_ai_enhance(image_file):
    """真正 AI 影像擴展 (Outpainting) + 直相修正"""
    ...
    with st.spinner("🚀 Manna AI 正在進行生成式影像擴展..."):
        try:
            ...
            prompt = "Outpaint this image into a cinematic 16:9 landscape banner. Extend the environment naturally to left and right sides. Keep style consistent."
            ...
            if result:
                ...
                for part in parts:
                    if 'inlineData' in part:
                        ...
                        log_debug("AI Expansion Complete.", "success")
                        return Image.open(io.BytesIO(img_data))
            
            log_debug("AI expansion returned no data. Using corrected original.", "warning")
            return img
        except Exception:
...
def apply_styles():
    st.markdown("""
        ...
        /* Debug Terminal - Permanent */
        .debug-terminal { background: #1E1E1E; color: #00FF00; padding: 12px; font-family: 'Courier New', monospace; font-size: 11px; border-top: 4px solid #FF0000; border-radius: 10px 10px 0 0; max-height: 200px; overflow-y: auto; margin-top: 50px;}
    ...
    with tab1:
        ...
        with cl:
            ...
            if p := st.chat_input("詢問項目細節..."):
                ...
                res = call_gemini_rest(f"SOW Context: {st.session_state.scope_of_word}\nInquiry: {p}")
                if res:
                    reply = res.get('candidates', [{}])[0].get('content', {}).get('parts', [{}])[0].get('text', "AI Timeout.")
            ...
            if files:
                ...
                for i, f in enumerate(files):
                    with cols[i%4]:
                        ...
                        if is_processed: st.markdown('<div class="ai-status-tag">✨ AI READY</div>', unsafe_allow_html=True)
